...
const db = new Proxy({}, {
  get(target, prop) {
    return require("@/db").db[prop];
  }
}) as any;
import webpush from "web-push";
import { sendPushToAll, sendPushToUser } from "@/utils/webpush";
...
describe("sendPushToAll", () => {
  ...
  it("does nothing when VAPID keys not configured (module not yet initialized)", async () => {
    ...
    const { sendPushToAll: freshSendPushToAll } = await import("@/utils/webpush");
  ...
  it("sends to all subscriptions when VAPID configured", async () => {
    ...
    vi.resetModules();
    const { sendPushToAll: freshSend } = await import("@/utils/webpush");
...
describe("sendPushToUser", () => {
  it("does nothing when VAPID keys not configured", async () => {
    ...
    delete process.env.VAPID_PRIVATE_KEY;
    vi.resetModules();
    const { sendPushToUser: freshSend } = await import("@/utils/webpush");
    const freshWebpush = (await import("web-push")) as unknown as { sendNotification: any };
