    ...
    async function loadData() {
      try {
        ...
        if (catRes.ok && artRes.ok) {
          ...
          const art = artData.article || artData;

          setCategories(catData || []);
          setTitle(art.title);
          setDescription(art.description);
          setPrice(String(art.price));
          setOldPrice(art.oldPrice ? String(art.oldPrice) : "");
          setStock(String(art.stock));
          setState(art.state);
    ...
    return (
      <div className="flex flex-col md:flex-row min-h-screen bg-ivoire text-encre dark:bg-encre dark:text-encre dark:text-ivoire font-sans">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <p className="animate-pulse font-medium text-or">Chargement des détails de l'article...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex justify-between items-center mb-10 pb-6 border-b border-beige/45 dark:border-anthracite/60">
          <div>
            <h1 className="font-display text-3xl font-bold tracking-tight">
              Modifier l' <span className="text-or italic font-light">Article</span>
            </h1>
            <p className="text-xs text-encre/65 dark:text-encre dark:text-ivoire/65 mt-1">
              Modifiez les détails de la pièce unique ID: {id}
            </p>
          </div>
          <a
            href="/admin/articles"
