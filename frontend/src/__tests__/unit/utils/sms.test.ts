...
vi.mock("@/backend/utils/webpush", () => ({
  sendPushToAll: vi.fn().mockResolvedValue(undefined),
}));
vi.mock("@/backend/utils/email", () => ({
  sendNewArticleEmail: vi.fn().mockResolvedValue(undefined),
}));

import { db } from "@/backend/db";
import { sendPushToAll } from "@/backend/utils/webpush";
import { sendNewArticleEmail } from "@/backend/utils/email";
import { sendSMSToAll, notifyNewStock } from "@/backend/utils/sms";
